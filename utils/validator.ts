...
const validator = {
  ...
  validateTransaction(transaction: { type: string, amount: number, fromWallet?: string, toWallet?: string }) {
    const schema = Joi.object({
      type: Joi.string().valid('deposit', 'withdraw', 'transfer').required(),
      amount: Joi.number().positive().required(),
      fromWallet: Joi.string().when('type', { is: 'transfer', then: Joi.required() }), // Adjust here
      toWallet: Joi.string().when('type', { is: 'transfer', then: Joi.required() }),   // Adjust here
